        ...
        if ((!image || options & SDWebImageRefreshCached) && (![self.delegate respondsToSelector:@selector(imageManager:shouldDownloadImageForURL:)] || [self.delegate imageManager:self shouldDownloadImageForURL:url]))
        {
            ...
            {                
                ...
                else
                {
                    ...
                    {
#if 1 // :TODO: In order to keep memory reasonable this patch resizes the image. It should have better logic as images can appear skewed.
                        if (downloadedImage)
                        {
                            if (
                                downloadedImage.size.width > 192.0f ||
                                downloadedImage.size.height > 192.0f
                                )
                            {
                                downloadedImage = [downloadedImage thumbnailImage:192.0f
